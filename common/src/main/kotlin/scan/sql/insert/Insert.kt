...
value class Insert(@PublishedApi internal val data:QueryData){
    inline fun <reified T:Table, reified V:DTO> colStr(field:KProperty1<T, String>, bind:KProperty1<V, String>):InsertValues{
        ...
        data.values().add(Field(FieldType.Column,T::class.tableName(), field.name, bind.name))
    ...
    inline fun <reified T:Table, reified V:DTO> colDTO(field:KProperty1<T, DTO>, bind:KProperty1<V, DTO>):InsertValues{
    ...
    inline fun <reified T:Table> cols(vararg field:KProperty1<T, *>):InsertSelect{
        ...
        data.values().addAll(field.map{Field(FieldType.Column,T::class.tableName(), it.name, null)})
