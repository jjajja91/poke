...
inline fun <reified T:Table, reified V:VO> Where.InStr(field:KProperty1<T, String>, bind:KProperty1<V, ArrayList<String>>):WhereAnd{
    data.addBind<V>(Bind(bind.name))
    data.where().add(In(T::class.simpleName!!, field.name, bind.name))
    return WhereAnd(data)
}
inline fun <reified T:Table, reified V:VO> Where.InNumber(field:KProperty1<T, Number>, bind:KProperty1<V, ArrayList<out Number>>):WhereAnd{
    data.addBind<V>(Bind(bind.name))
    data.where().add(In(T::class.simpleName!!, field.name, bind.name))
    return WhereAnd(data)
}