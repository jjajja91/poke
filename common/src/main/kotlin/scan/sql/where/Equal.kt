...
inline fun <reified T:Table, reified V:VO> Where.equalStr(field:KProperty1<T, String>, bind:KProperty1<V, String>):WhereAnd{
    data.addBind<V>(Bind(bind.name))
    data.where().add(Equal(T::class.simpleName!!, field.name, bind.name))
    return WhereAnd(data)
}
inline fun <reified T:Table, reified V:VO> Where.equalNum(field:KProperty1<T, Number>, bind:KProperty1<V, Number>):WhereAnd{
    data.addBind<V>(Bind(bind.name))
    data.where().add(Equal(T::class.simpleName!!, field.name, bind.name))
    return WhereAnd(data)
}