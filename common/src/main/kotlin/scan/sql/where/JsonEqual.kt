...
inline fun <reified T:Table, reified TV:VO, reified V:VO> Where.JsonEqualStr(field:KProperty1<T, TV>, key: KProperty1<TV, String>, bind:KProperty1<V, String>):WhereAnd{
    data.addBind<V>(Bind(bind.name){ it.toString() })
    data.where().add(JsonEqual(T::class.simpleName!!, field.name, key.name, bind.name))
